...
const retrySequelizeConnection = async (retries = 10, delay = 5000): Promise<void> => {
  while (retries > 0) {
    try {
...
await app
  ...
  .register(socketioServer, {} as any)
...
app.get("/api", (_request: FastifyRequest, reply: FastifyReply) => {
